...
def build_mlp(
    ...
    for l in range(n_layers):
        ...
        if l == n_layers-1:
            out = layers.fully_connected(out, int(output_size), activation_fn=output_activation)
...
class PG(object):
    ...
    def build_policy_network_op(self, scope="policy_network"):
        ...
        with tf.variable_scope(scope) as scope:
            if self.discrete:
                ...
                self.sampled_action = tf.squeeze(tf.multinomial(action_logits, 1), axis = 1)# TODO What is num_samples?
                ...
                log_std = tf.get_variable("log_std", [1], dtype = tf.float32)
                self.sampled_action = tf.random_normal([self.action_dim],
                                                       action_means,
                                                       tf.exp(log_std))
                distr = ds.MultivariateNormalDiag(
                    loc = action_means,
                    scale_diag = tf.ones(self.action_dim) * tf.exp(log_std)
                )
                self.logprob = distr.log_prob(self.action_placeholder, name = "log_prob")
            #######################################################
            #########          END YOUR CODE.          ############
    ...
    def add_baseline_op(self, scope="baseline"):
        ...
        baseline_out = build_mlp(self.observation_placeholder, 1, scope)
        self.baseline = tf.squeeze(baseline_out)
        self.baseline_target_placeholder = self.advantage_placeholder
        # print("self.baseline: " + str(self.baseline.shape))
        # print("self.baseline_target_placeholder: " + str(self.baseline_target_placeholder))
        baseline_loss = tf.losses.mean_squared_error(
            labels = self.baseline_target_placeholder,
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            ...
            path_returns = rewards * (self.config.gamma ** np.arange(0.0, path_length))
            returns = np.cumsum(path_returns[::-1])[::-1]
            #######################################################
            #########          END YOUR CODE.          ############
    ...
    def calculate_advantage(self, returns, observations):
        ...
        #########   YOUR CODE HERE - 5-10 lines.   ############
        if self.config.use_baseline:
            base = self.sess.run(self.baseline,
    ...
    def train(self):
        ...
        for t in range(self.config.num_batches):
            ...
            # print("observation_placeholder" + str(self.observation_placeholder.shape))
            # print("observations" + str(observations.shape))
            # print("advantage_placeholder" + str(self.advantage_placeholder.shape))
            # print("advantages" + str(advantages.shape))
            # print("action_placeholder" + str(self.action_placeholder.shape))
            ...
            msg = " {:3} Average reward: {:04.2f} +/- {:04.2f}".format(t, avg_reward, sigma_reward)
